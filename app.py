...
def upload_bot():
    ...
    total_current = current_bots + website_count + cli_count
...
def mini_web_view(type, id):
    """Mini Web View - Limited access, no login required"""
...
def mini_web_action(type, id, action):
    """Mini Web API - Start/Stop/Restart only"""
        ...
        <div class="user-container">
            ...
            <div id="tab-cli" class="tab-content">
                ...
                <div class="cli-terminal-container">
                    <div class="cli-terminal-header"><span><i class="fa-solid fa-terminal"></i> CLI Terminal</span><span id="cliStatusBadge" class="status-badge status-stopped">● STOPPED</span></div>
    ...
    (function(){
        ...
        async function handleLogin(){
            ...
            try{
                ...
                if(data.success){currentUser={username:data.username,role:data.role};location.reload();}else{loginError.textContent=data.error||'Invalid credentials';}
        ...
        function renderBots(bots){
            ...
            bots.forEach(bot=>{
                ...
                html+=`<div class="bot-card" data-id="${bot.id}"><div class="bot-header"><span class="bot-name">${escapeHtml(bot.filename)}</span><span class="bot-status ${statusClass}">● ${bot.status.toUpperCase()}</span></div><div class="bot-owner">👤 ${escapeHtml(bot.user)}</div><div class="bot-uptime" id="uptime-${bot.id}">UPTIME: ${uptimeDisplay}</div><div class="bot-controls"><button class="btn-start" data-action="start">${bot.status==='running'?'▶ RUNNING':'▶ START'}</button><button class="btn-stop" data-action="stop">⏹ STOP</button><button class="btn-edit" data-action="edit">✎ EDIT</button><button class="btn-restart" data-action="restart">⟳ RESTART</button><button class="btn-download" data-action="download">⬇ DOWNLOAD</button><button class="btn-delete" data-action="delete">🗑 DELETE</button>${(hasToken&&botUsername)?`<button class="btn-openbot" data-action="openbot" data-bot="${botUsername}">🤖 Open Bot</button>`:''}<button class="btn-miniweb" onclick="window.open('/mini/bot/${bot.id}','_blank')">📱 Mini Web</button></div></div>`;
            });
            ...
            document.querySelectorAll('.bot-card [data-action]').forEach(btn=>{
                btn.addEventListener('click',async function(e){
                    e.stopPropagation();const action=this.dataset.action;const card=this.closest('.bot-card');const botId=card.dataset.id;
        ...
        function attachFullListEvents(){
            document.querySelectorAll('#fullUserListContainer [data-action]').forEach(btn=>{
                btn.addEventListener('click',async function(e){
                    e.stopPropagation();const action=this.dataset.action;const username=this.dataset.username;const card=this.closest('.list-item');
                    if(action==='setLimit'){const input=card.querySelector('.limit-input');const val=parseInt(input.value,10);if(isNaN(val)||val<0){await customAlert('Enter a valid number.','⚠️');return;}try{await apiCall(`/api/users/${username}`,{method:'PUT',body:JSON.stringify({limit:val})});await loadAdminUsers();}catch(e){await customAlert(e.message,'❌');}}
                    else if(action==='toggleBan'){const user=(await apiCall('/api/users')).find(u=>u.username===username);if(!user)return;try{await apiCall(`/api/users/${username}`,{method:'PUT',body:JSON.stringify({banned:!user.banned})});await loadAdminUsers();}catch(e){await customAlert(e.message,'❌');}}
                    ...
                    else if(action==='setExpiry'){const expiryInput=card.querySelector('[data-field="newExpiry"]');const expiry=expiryInput.value.trim();try{await apiCall(`/api/users/${username}`,{method:'PUT',body:JSON.stringify({expiry:expiry})});expiryInput.value='';await customAlert('Expiry updated.','✅');await loadAdminUsers();}catch(e){await customAlert(e.message,'❌');}}
                    else if(action==='deleteUser'){const confirmed=await customConfirm(`Delete user ${username} and all their bots?`,'🗑️');if(!confirmed)return;if(username===currentUser.username){await customAlert('Cannot delete yourself.','🚫');return;}try{await apiCall(`/api/users/${username}`,{method:'DELETE'});await loadAdminUsers();}catch(e){await customAlert(e.message,'❌');}}
                });
            });
        }
        function renderSimpleLists(users){
            ...
            document.querySelectorAll('.btn-remove-simple').forEach(btn=>{
                btn.addEventListener('click',async function(e){
                    e.stopPropagation();const username=this.dataset.username;const confirmed=await customConfirm(`Remove user ${username}?`,'🗑️');if(!confirmed)return;if(username===currentUser.username){await customAlert('Cannot remove yourself.','🚫');return;}try{await apiCall(`/api/users/${username}`,{method:'DELETE'});await loadAdminUsers();}catch(e){await customAlert(e.message,'❌');}
                });
            });
        }
        ...
        console.log('📱 Mini Web view available for websites and bots.');
    })();
    </script>
</body>
</html>
...
if __name__ == '__main__':
    ...
    print("="*60)
    print("🚀 YUVICODEX ULTIMATE (Websites + Bots + CLI + Mini Web)")
    print(f"🌐 Port: {port}")
