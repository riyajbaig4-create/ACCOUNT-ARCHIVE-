...
# ---------- PROCESS TRACKING (for bots, websites, cli) ----------
processes = {}
cli_processes = {}

# ---------- SQLITE FOR WEBSITES AND CLI ----------
...
def api_list_cli_tools():
    """List all CLI tools for the current user"""
    username = session['username']
    with get_db() as conn:
...
@login_required
def upload_cli():
    """Upload CLI tool - ZIP or individual files"""
    username = session['username']
...
def cli_tool_start(tool_id):
    """Start a CLI tool with interactive terminal"""
    with get_db() as conn:
...
def cli_tool_delete(tool_id):
    """Delete a CLI tool and all its files"""
    with get_db() as conn:
        tool = conn.execute('SELECT * FROM cli_tools WHERE id = ? AND owner_username = ?',
                           (tool_id, session['username'])).fetchone()
    if not tool:
...
def cli_tool_download(tool_id):
    """Download entire CLI tool folder as ZIP"""
    with get_db() as conn:
        tool = conn.execute('SELECT * FROM cli_tools WHERE id = ? AND owner_username = ?',
                           (tool_id, session['username'])).fetchone()
    if not tool:
...
def cli_tool_send_input(tool_id):
    """Send input to a running CLI tool"""
    data = request.json
...
def cli_tool_upload_file(tool_id):
    """Upload file to CLI tool folder and send path to CLI"""
...
def deploy_bot_logs_sse(bot_id):
    """Real-time bot logs using Server-Sent Events"""
    bot = bots_db.get(bot_id)
    if not bot:
        abort(404)
            ...
            <div id="tab-cli" class="tab-content">
                ...
                <div class="cli-terminal-container">
                    <div class="cli-terminal-header">
                        ...
                        <span id="cliStatusBadge" class="status-badge" style="color:#aaa;font-size:12px;">● STOPPED</span>
                    </div>
    ...
    <script>
        (function() {
            ...
            async function saveSettings() {
                const payload = { website_name: settingsWebsiteName.value.trim() || 'YUVICODEX', social_links: {
            ...
            cliClearBtn.addEventListener('click', function() {
                cliTerminal.textContent = 'Terminal cleared.';
            });
            ...
            function customPrompt(message, defaultValue) {
                return new Promise((resolve) => {
                    ...
                    showCustomModal('✏️', bodyHTML, [
                        ...
                        { label: 'OK', value: true, className: 'btn-confirm' }
                    ]).then((result) => { if (result === null) resolve(null); else { const val = document.getElementById('promptInput')?.value; resolve(val); } });
                });
            }
...
# ============================================================
# MAIN ROUTES FOR USER AUTH, PROFILE, SETTINGS, STATS, ETC.
# ============================================================
...
def create_user():
    ...
    new_user = {'username': username, 'password': password, 'role': role, 'limit': limit, 'banned': False, 'expires_at': expires_at, 'session_version': 0}
...
def delete_bot(bot_id):
    ...
    # Remove folder if no other bots in that project
...
def upload():
        ...
        for root, dirs, files_in_folder in os.walk(project_folder):
            for fname in files_in_folder:
                ...
                if interpreter:
                    ...
                    bot = {'user': username, 'project': project_id, 'filename': fname, 'status': 'stopped', 'pid': None, 'start_time': None, 'interpreter': interpreter}
                    bots_db[bot_id] = bot
    ...
    return jsonify({'success': True, 'project_id': project_id, 'bots_created': len(created_bots)})

# ---------- WEBSITE MANAGEMENT ROUTES (existing) ----------
...
def api_stats():
    ...
    if RENDER_API_KEY:
        ...
        if services:
            ...
            for svc in services:
                ...
                render_services.append({'name': svc.get('name', 'Unnamed'), 'type': svc.get('type', 'web'), 'status': status, 'uptime_hours': 0})
    ...
    return jsonify({
        ...
        'ram': {'used_mb': used_mb, 'total_mb': total_mb, 'percent': ram_percent},
...
def list_files():
    ...
    if os.path.isfile(abs_path):
        return jsonify({'type': 'file', 'name': os.path.basename(abs_path), 'path': path, 'size': os.path.getsize(abs_path), 'modified': os.path.getmtime(abs_path)})
    items = []
    try:
        for entry in os.listdir(abs_path):
            full = os.path.join(abs_path, entry)
            rel = os.path.relpath(full, BASE_DIR)
            items.append({'name': entry, 'path': rel, 'type': 'directory' if os.path.isdir(full) else 'file', 'size': os.path.getsize(full) if os.path.isfile(full) else 0, 'modified': os.path.getmtime(full)})
...
# ---------- TEMPLATES ----------
ERROR_TEMPLATE = """<!DOCTYPE html><html><head><title>Error</title><style>body{background:#0a0e1a;color:#fff;font-family:system-ui;display:flex;justify-content:center;align-items:center;height:100vh}.card{background:rgba(255,255,255,0.05);padding:40px;border-radius:20px;text-align:center}h1{color:#ff4757}a{color:#00e5ff;text-decoration:none}</style></head><body><div class="card"><h1>{{ message }}</h1><p>Slug: {{ slug }}</p><a href="/dashboard">← Dashboard</a></div></body></html>"""
