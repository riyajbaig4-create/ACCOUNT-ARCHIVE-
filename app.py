...
# ---------- Modified Start Process with Runtime Tracking ----------
def start_website_process(website_id, log_callback=None):
    ...
    try:
        ...
        # Set last_start_time and status to starting
        with get_db() as conn:
            conn.execute('UPDATE websites SET last_start_time = CURRENT_TIMESTAMP, status = ? WHERE id = ?',
                         ('starting', website_id))
            conn.commit()
        ...
        if detected_port and log_callback:
...
# ---------- Modified Stop Process with Runtime Tracking ----------
def stop_website_process(website_id):
    ...
    # Calculate elapsed time and add to total_runtime_seconds
    last_start = website['last_start_time']
    if last_start:
        try:
            # last_start is string like '2025-01-01 12:00:00'
            start_dt = datetime.fromisoformat(last_start.replace(' ', 'T'))
            elapsed = int((datetime.now() - start_dt).total_seconds())
            with get_db() as conn:
                conn.execute('UPDATE websites SET total_runtime_seconds = total_runtime_seconds + ? WHERE id = ?',
                             (elapsed, website_id))
                conn.commit()
        except Exception as e:
            log_website(website_id, f"Failed to update runtime: {str(e)}", 'error')
    ...
    with get_db() as conn:
        conn.execute('UPDATE websites SET last_start_time = NULL, last_stopped = CURRENT_TIMESTAMP WHERE id = ?',
                     (website_id,))
        conn.commit()
...
# ---------- API for System Stats (Owner only) ----------
@app.route('/api/stats')
def api_stats():
    if 'user_id' not in session:
    ...
    # Sum of all website runtime (including running ones)
    with get_db() as conn:
        rows = conn.execute('SELECT id, total_runtime_seconds, last_start_time, status FROM websites').fetchall()
    total_website_seconds = 0
    for row in rows:
        total_website_seconds += row['total_runtime_seconds'] or 0
        if row['status'] == 'running' and row['last_start_time']:
            try:
                start_dt = datetime.fromisoformat(row['last_start_time'].replace(' ', 'T'))
                elapsed = int((datetime.now() - start_dt).total_seconds())
                total_website_seconds += elapsed
            except:
                pass

    total_seconds = main_uptime_seconds + total_website_seconds
    total_hours = total_seconds / 3600.0
    main_hours = main_uptime_seconds / 3600.0
    website_hours = total_website_seconds / 3600.0
    ...
    return jsonify({
        'main_uptime_seconds': main_uptime_seconds,
        'websites_total_seconds': total_website_seconds,
        'total_seconds': total_seconds,
        'total_hours': round(total_hours, 2),
        'main_hours': round(main_hours, 2),
        'website_hours': round(website_hours, 2),
        'disk': {
        ...
        'ram': {
            'total_gb': round(ram_total, 2) if ram_total != 'N/A' else 'N/A',
            'used_gb': round(ram_used, 2) if ram_used != 'N/A' else 'N/A',
            'percent': ram_percent
        },
...
</style></head><body><div class="glass"><h1>⚠️ {{ message }}</h1><p>Slug: <strong>{{ slug }}</strong></p><a href="/dashboard">← Go to Dashboard</a></div></body></html>"""
...
# Modified DASHBOARD_TEMPLATE with Stats button for admin
DASHBOARD_TEMPLATE = """
<!DOCTYPE html>
<html>
...
<!-- Stats Modal (Owner only) -->
<div class="modal-overlay" id="statsModal">
<div class="modal">
<button class="modal-close" id="statsModalClose">&times;</button>
<h2>📊 System Statistics</h2>
<div class="stats-grid" id="statsGrid">
<div class="stat-card"><div class="label">Total Hours Used (All)</div><div class="value" id="statTotalHours">--</div><div class="sub">Main + Websites</div></div>
...
<div class="stat-card"><div class="label">Websites Runtime (Sum)</div><div class="value" id="statWebsiteHours">--</div><div class="sub">All managed sites</div></div>
<div class="stat-card"><div class="label">Number of Websites</div><div class="value" id="statWebsitesCount">--</div><div class="sub">Total deployed</div></div>
<div class="stat-card"><div class="label">Disk Usage</div><div class="value" id="statDiskUsed">--</div><div class="sub">Free: <span id="statDiskFree">--</span> GB / Total: <span id="statDiskTotal">--</span> GB</div><div class="progress-bar"><div class="fill" id="diskFill" style="width:0%;"></div></div></div>
<div class="stat-card"><div class="label">CPU Usage</div><div class="value" id="statCpu">--</div><div class="sub">Percent</div></div>
<div class="stat-card"><div class="label">RAM Usage</div><div class="value" id="statRam">--</div><div class="sub">Used: <span id="statRamUsed">--</span> GB / Total: <span id="statRamTotal">--</span> GB</div><div class="progress-bar"><div class="fill" id="ramFill" style="width:0%;"></div></div></div>
</div>
</div>
</div>
...
if(!files.length)return alert('Select at least one file (ZIP or individual)');
...
statsModal.addEventListener('click', function(e) {
    if (e.target === this) { statsModal.classList.remove('open'); if (statsInterval) { clearInterval(statsInterval); statsInterval = null; } }
});
...
function fetchStats() {
    ...
    .then(data => {
        ...
        document.getElementById('statWebsiteHours').textContent = data.website_hours + ' hrs';
        document.getElementById('statWebsitesCount').textContent = data.websites_count;
        document.getElementById('statDiskUsed').textContent = data.disk.used_gb + ' GB';
        document.getElementById('statDiskFree').textContent = data.disk.free_gb;
        document.getElementById('statDiskTotal').textContent = data.disk.total_gb;
        const diskPercent = (data.disk.used_gb / data.disk.total_gb) * 100;
        document.getElementById('diskFill').style.width = Math.min(diskPercent, 100) + '%';
        document.getElementById('statCpu').textContent = data.cpu_percent + '%';
        document.getElementById('statRam').textContent = data.ram.percent + '%';
        document.getElementById('statRamUsed').textContent = data.ram.used_gb;
        document.getElementById('statRamTotal').textContent = data.ram.total_gb;
